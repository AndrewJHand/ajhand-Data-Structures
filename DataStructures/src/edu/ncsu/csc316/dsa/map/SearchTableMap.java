...
public class SearchTableMap<K extends Comparable<K>, V> extends AbstractOrderedMap<K, V> {
	...
	/**
	 * the list that holds all the entries
	 */
    ...
    public V get(K key) {
        ...
        if(index >= 0 && index < list.size() && compare(list.get(index).getKey(), key) == 0) {
    ...
    public V put(K key, V value) {
        ...
        if(index < list.size() && list.get(index).getKey().compareTo(key) == 0) {
    ...
    public V remove(K key) {
        ...
        if(index >= 0 && compare(list.get(index).getKey(), key) == 0) {
        	V val = list.get(index).getValue();
        	list.remove(index);
        	return val;
        }
