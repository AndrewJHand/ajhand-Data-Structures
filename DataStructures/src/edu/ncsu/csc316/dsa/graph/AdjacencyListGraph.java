...
public class AdjacencyListGraph<V, E> extends EdgeListGraph<V, E> {
    ...
    public Iterable<Edge<E>> incomingEdges(Vertex<V> vertex) {
    	...
    	return super.incomingEdges(vert);
//        if(isDirected()) {
//        	
//        }
//        else {
...
//        }
    ...
    public Edge<E> insertEdge(Vertex<V> vertex1, Vertex<V> vertex2, E edgeData) {
        ALVertex v1 = validate(vertex1);
        ALVertex v2 = validate(vertex2);
        //ALEdge e = validate(edgeData);
        ALEdge edge = validate(super.insertEdge(v1, v2, edgeData));
        edge.getIncomingListPosition();
        edge.getOutgoingListPosition();
        edge.setIncomingListPosition(v1.getIncoming().addLast(edge));
        edge.setOutgoingListPosition(v2.getOutgoing().addLast(edge));
        edge.setIncomingListPosition(v2.getIncoming().addLast(edge));
        edge.setOutgoingListPosition(v1.getOutgoing().addLast(edge));
        return edge;
        //v1.getIncoming().se
    }
    ...
    public int outDegree(Vertex<V> vertex) {
    	ALVertex vert = validate(vertex);
        return super.outDegree(vert);
    }

    @Override
    public Iterable<Edge<E>> outgoingEdges(Vertex<V> vertex) {
    	ALVertex vert = validate(vertex);
    	return super.outgoingEdges(vert);
    }
    ...
    public void removeEdge(Edge<E> edge) {
    	ALEdge e = validate(edge);
        super.removeEdge(e);
        e.setIncomingListPosition(null);
        e.setOutgoingListPosition(null);
    }
     ...
     * @param e is the edge to validate
     *                                  edge
     */
